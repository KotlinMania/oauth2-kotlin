# Claude Code Project Instructions — oauth2-kotlin

## Project Overview

This is **oauth2-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`oauth2`](https://crates.io/crates/oauth2).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/oauth2/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/oauth2>
